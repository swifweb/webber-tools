    ...
    public enum Command {
        ...
        public func arguments(tripleWasm: Bool = true) -> [String] {
            ...
            case .build(let r, let p):
                ...
                if tripleWasm {
                    args.append(contentsOf: ["--triple", "wasm32-unknown-wasi", "-Xswiftc", "-DJAVASCRIPTKIT_WITHOUT_WEAKREFS", "-Xswiftc", "-Xclang-linker", "-Xswiftc", "-mexec-model=reactor", "-Xlinker", "-lCoreFoundation", "-Xlinker", "-licuuc", "-Xlinker", "-licui18n", "-Xlinker", "--stack-first", "-Xlinker", "--export=main", "--build-path", "./.build/.wasi"])
                    return args
                } else {
